...
type Container struct {
	...
	lanTransport      *lan.LANTransport
	wanTransport      *wan.WANTransport
...
func (c *Container) LANTransport() *lan.LANTransport {
	return c.lanTransport
}

func (c *Container) WANTransport() *wan.WANTransport {
	return c.wanTransport
}
...
func (c *Container) initUseCases() error {
	...
	if c.lanTransport != nil {
		c.lanTransport.SetChunkHandler(chunkHandler)
		log.Println("✓ Chunk handler bağlandı (LAN)")

		// Connection request callback'ini bağla (sadece LAN için)
		connMgr := c.lanTransport.GetTCPConnectionManager()
...
func (c *Container) initP2PTransport() error {
	...
	// LAN Transport oluştur ve başlat (her zaman aktif)
	...
	if wanTransport != nil {
		c.transportProvider = p2p.NewHybridTransportProvider(lanTransport, wanTransport)
		log.Printf("✓ P2P Transport başlatıldı (LAN + WAN, device: %s, port: %d)", deviceName, p2pPort)
	} else {
		// Chunk handler daha sonra bağlanacak
		c.transportProvider = lanTransport
		log.Printf("✓ P2P Transport başlatıldı (yalnızca LAN, device: %s, port: %d)", deviceName, p2pPort)
	}
